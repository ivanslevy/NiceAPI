...
| 設定 | プロバイダー | グループ |
| :---: | :---: | :---: |
| ![設定](example/settings_ja.png) | ![プロバイダー](example/providers_ja.png) | ![グループ](example/groups_ja.png) |
| UI言語やフェイルオーバーの再試行ロジックなど、グローバル設定を構成します。 | 資格情報、コスト、強力なモデルインポーターなど、すべてのAIプロバイダーエンドポイントを管理します。 | 複数のプロバイダーをグループ化して、優先度ベースのフェイルオーバー機能を備えた仮想エンドポイントを作成します。 |

| コールログ | APIキー | 失敗キーワード |
| :---: | :---: | :---: |
| ![コールログ](example/call_logs_ja.png) | ![APIキー](example/api_keys_ja.png) | ![失敗キーワード](example/fail_keywords_ja.png) |
| すべてのAPIコールの詳細なログを検査します。ログをクリックすると、完全なレスポンスの内容、およびコストの詳細が表示されます。 | きめ細かなアクセス制御のために、APIキーを生成して特定のグループに割り当てます。 | プロバイダーの応答で見つかった場合に自動フェイルオーバーをトリガーするキーワードを定義します。 |

| ダッシュボード |
| :---: |
| ![ダッシュボード](example/dashboard_ja.png) |
| リアルタイムのAPI使用状況、成功率、パフォーマンスメトリクスを一目で監視します。 |
