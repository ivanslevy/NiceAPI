...
| 설정 | 공급자 | 그룹 |
| :---: | :---: | :---: |
| ![설정](example/settings_ko.png) | ![공급자](example/providers_ko.png) | ![그룹](example/groups_ko.png) |
| UI 언어 및 장애 조치 재시도 로직과 같은 전역 설정을 구성합니다. | 자격 증명, 비용 및 강력한 모델 가져오기 도구를 포함하여 모든 AI 공급자 엔드포인트를 관리합니다. | 여러 공급자를 함께 그룹화하여 우선순위 기반 장애 조치 기능이 있는 가상 엔드포인트를 만듭니다. |
...
| 대시보드 |
| :---: |
| ![대시보드](example/dashboard_ko.png) |
| 실시간 API 사용량, 성공률 및 성능 지표를 한눈에 모니터링합니다. |
